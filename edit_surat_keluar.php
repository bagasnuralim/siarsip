<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['no_agenda'] == "" || $_REQUEST['no_surat'] == "" || $_REQUEST['tujuan'] == "" || $_REQUEST['isi'] == "" || $_REQUEST['referensi_suratk'] == ""
                || $_REQUEST['status_publishk'] == "" || $_REQUEST['kode'] == "" || $_REQUEST['tgl_surat'] == ""  || $_REQUEST['keterangan'] == ""){
                ...
                $isi = $_REQUEST['isi'];
                $referensi_surat = $_REQUEST['referensi_suratk'];
                $status_publish = $_REQUEST['status_publishk'];
                ...
                if(!preg_match("/^[0-9]*$/", $no_agenda)){
                    ...
                    if(!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $no_surat)){
                        ...
                        if(!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $tujuan)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $referensi_surat)){
                                $_SESSION['referensi_suratk'] = 'Form Referensi Surat hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {
                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $status_publish)){
                                $_SESSION['status_publishk'] = 'Form Status Publish hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {
                                    ...
                                    if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                        ...
                                        if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $keterangan)){
                                            ...
                                            if($file != ""){
                                                ...
                                                if(in_array($eks, $ekstensi) == true){
                                                    if($ukuran < 2500000){
                                                        ...
                                                        if(!empty($file)){
                                                            ...
                                                            $query = mysqli_query($config, "UPDATE tbl_surat_keluar SET no_agenda='$no_agenda',
                                                            tujuan='$tujuan',
                                                            no_surat='$no_surat',
                                                            isi='$isi',
                                                            referensi_surat='$referensi_surat',
                                                            status_publish='$status_publish',
                                                            kode='$nkode',tgl_surat='$tgl_surat',file='$nfile',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
                                                ...
                                                referensi_surat='$referensi_surat',
                                                status_publish='$status_publish',
                                                kode='$nkode',tgl_surat='$tgl_surat',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
            ...
            $query = mysqli_query($config, "SELECT id_surat, no_agenda, tujuan, no_surat, isi,referensi_surat,status_publish, kode, tgl_surat, file, keterangan, id_user FROM tbl_surat_keluar WHERE id_surat='$id_surat'");
            list($id_surat, $no_agenda, $tujuan, $no_surat, $isi,$referensi_surat,$status_publish, $kode, $tgl_surat, $file, $keterangan, $id_user) = mysqli_fetch_array($query);
                    ...
                    <form class="col s12" method="POST" action="?page=tsk&act=edit" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="input-field col s6">
                                ...
                                <input id="referensi_suratk" type="text" class="validate" name="referensi_suratk" value="<?php echo $referensi_surat ;?>" required>
                                    <?php
                                        if(isset($_SESSION['referensi_suratk'])){
                                            $isik = $_SESSION['referensi_suratk'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$referensi_surat.'</div>';
                                            unset($_SESSION['referensi_suratk']);
                                        }
                                    ?>
                                <label for="isi">Referensi</label>
                            </div>
                            <div class="input-field col s6">
                            ...
                            <div class="input-field col s11 right">
                                <select class="browser-default validate" name="status_publishk" id="status_publishk" required>
                                ...
                                <?php
                                    if(isset($_SESSION['status_publishk'])){
                                        $status_publish = $_SESSION['status_publishk'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$tipeuser.'</div>';
                                        unset($_SESSION['status_publishk']);
                                    }
                                ?>
                        </div>
                            <div class="input-field col s6">
