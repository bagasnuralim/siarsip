<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['no_agenda'] == "" || $_REQUEST['no_surat'] == "" || $_REQUEST['asal_surat'] == "" || $_REQUEST['isi'] == "" || $_REQUEST['referensi_surat'] == ""
                || $_REQUEST['status_publish'] == "" || $_REQUEST['kode'] == "" || $_REQUEST['indeks'] == "" || $_REQUEST['tgl_surat'] == ""  || $_REQUEST['keterangan'] == ""){
                ...
                $isi = $_REQUEST['isi'];
                $referensi_surat = $_REQUEST['referensi_surat'];
                $status_publish = $_REQUEST['status_publish'];
                ...
                if(!preg_match("/^[0-9]*$/", $no_agenda)){
                    ...
                    if(!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $no_surat)){
                        ...
                        if(!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $asal_surat)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $referensi_surat)){
                                $_SESSION['ereferensi'] = 'Form Referensi Surat hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {    
                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $status_publish)){
                                $_SESSION['status_publish'] = 'Form Status Publish hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {
                                    ...
                                    if(!preg_match("/^[a-zA-Z0-9., -]*$/", $indeks)){
                                        ...
                                        if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                            ...
                                            if($file != ""){
                                                ...
                                                if(in_array($eks, $ekstensi) == true){
                                                    if($ukuran < 2300000){
                                                        ...
                                                        if(!empty($file)){
                                                            ...
                                                            isi='$isi',
                                                            referensi_surat='$referensi_surat',
                                                            status_publish='$status_publish',
                                                            kode='$nkode',indeks='$indeks',tgl_surat='$tgl_surat',file='$nfile',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
                                                            ...
                                                            isi='$isi',
                                                            referensi_surat='$referensi_surat',
                                                            status_publish='$status_publish',kode='$nkode',indeks='$indeks',tgl_surat='$tgl_surat',file='$nfile',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
                                                ...
                                                referensi_surat='$referensi_surat',
                                                status_publish='$status_publish',
                                                kode='$nkode',indeks='$indeks',tgl_surat='$tgl_surat',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
        ...
        $query = mysqli_query($config, "SELECT id_surat, no_agenda, no_surat, asal_surat, isi, referensi_surat, status_publish, kode, indeks, tgl_surat, file, keterangan, id_user FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
        list($id_surat, $no_agenda, $no_surat, $asal_surat, $isi,$referensi_surat, $status_publish, $kode, $indeks, $tgl_surat, $file, $keterangan, $id_user) = mysqli_fetch_array($query);
            ...
            <div class="row jarak-form">
                ...
                <form class="col s12" method="POST" action="?page=tsm&act=edit" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="input-field col s6">
                            <i class="material-icons prefix md-prefix">dvr</i>
                            <input id="referensi_surat" type="text" class="validate" name="referensi_surat" value="<?php echo $referensi_surat ;?>" required>
                                <?php
                                    if(isset($_SESSION['ereferensi_surat'])){
                                        $eisi = $_SESSION['ereferensi_surat'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$eisi.'</div>';
                                        unset($_SESSION['ereferensi_surat']);
                                    }
                                ?>
                            <label for="isi">Referensi Surat</label>
                        </div>
                        <div class="input-field col s6">
                        ...
                        <div class="input-field col s6">
                            <i class="material-icons prefix md-prefix">accessibility</i><label>Status Publish</label><br/>
                            <div class="input-field col s11 right">
                                <select class="browser-default validate" name="status_publish" id="status_publish" required>
                                    <option value="<?php echo $status_publish ?>">
                                        <?php
                                            if($status_publish == 0){
                                                echo 'Public';
                                            } else {
                                ...
                                <?php
                                    if(isset($_SESSION['status_publish'])){
                                        $status_publish = $_SESSION['status_publish'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$tipeuser.'</div>';
                                        unset($_SESSION['status_publish']);
                                    }
                                ?>
                        </div>
                        <div class="input-field col s6">
