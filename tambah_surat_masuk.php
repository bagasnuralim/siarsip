<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['no_agenda'] == "" || $_REQUEST['no_surat'] == "" 
                || $_REQUEST['asal_surat'] == "" || $_REQUEST['isi'] == "" 
                || $_REQUEST['referensi_surat'] == "" || $_REQUEST['status_publish'] == "" 
                || $_REQUEST['kode'] == "" || $_REQUEST['indeks'] == "" 
                    ...
                    if(!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $no_surat)){
                        ...
                        if(!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $asal_surat)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $isi)){
                                ...
                                if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $referensi_surat)){
                                    $_SESSION['isi'] = 'Form Referensi Surat hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                    echo '<script language="javascript">window.history.back();</script>';
                                } else {
                                if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $status_publish)){
                ...
                <form class="col s12" method="POST" action="?page=tsm&act=add" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="input-field col s6">
                            ...
                            <input id="referensi_surat" type="text" class="validate" name="referensi_surat" required>
                                <?php
                                    if(isset($_SESSION['referensi_surat'])){
                                        $referensi_surat = $_SESSION['referensi_surat'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$referensi_surat.'</div>';
                                        unset($_SESSION['referensi_surat']);
                                    }
                                ?>
